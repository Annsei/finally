...
状态：**全部完成 2026-07-07**（CN-1 eb005ae / CN-2 1dc630c / CN-3 be4c2de /
CN-4 c849539）。路线 B（配置层 + 增量挂钩）落地，用户已确认。
交付：一份镜像两个市场——8800 us 视觉逐字节不变，8801（FINALLY_MARKET=cn）
全中文、红涨绿跌、¥100k、T+1、整手、14 只 A 股。最终 pytest 813 / jest 216 /
docker E2E us 11 + cn 3，现有测试零改动，浏览器双版本实测通过。

约束（已达成）：**不覆盖现有文件** —— 美股版行为逐字节不变，
